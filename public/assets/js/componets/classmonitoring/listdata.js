...
export function getDatalist() {
    ...
    export const getTampilback =(datas)=>{
     ...
     const getTampilData =(datas)=>{
            ...
            headers: { 'url': 'mont/listdatamanual' },
    ...
    const  settablelist=(result)=>{
          ...
          <table id="tabel1" class="table table-striped table-hover" style="width:100%">                    
            <thead id="thead" class="thead">
              <tr>
                ...
                <th>Tahun</th>
                <th>Bulan</th>
                <th>Sumber Informasi Pembelian</th>
                <th>Faktor  Pertimbangan Kampanye</th>
                <th class="text-end">Bayak kampanye</th>
                <th class="text-end">Bayak Kuesioner</th>
       ...
       $.each( result,function(a,item){
        const {MonitoringID,Tahun,Bulan,Kesimpulan_Kampanye,Kesimpulan_kuesioner,User_Input,total_kampanye,qtypemasangan,tanggal_update}=item;
        const nameamount =conversionMonth(Bulan);
         datatabel +=`<tr>
                        <td>${no++}</td>
                        <td>${Tahun}</td>
                        <td>${nameamount}</td>
                        <td>${Kesimpulan_Kampanye}</td>
                        <td>${Kesimpulan_kuesioner}</td>
                        <td class="text-end">${total_kampanye}</td>
                        <td class="text-end">${qtypemasangan}</td>
                        <td class="text-center">${tanggal_update}</td>
                        <td>${User_Input}</td>
                        <td><button type="button" data-id='${MonitoringID}' data-tahun='${Tahun}' data-bulan='${Bulan}' class="btn btn-info"  title="Edit" id="editdata"><i class="fa-solid fa-file-pen"></i></button> </td>
 ...
 datatabel += `
      ...
      $(document).on("click","#editdata",function(e){
        e.preventDefault();
        const MonitoringID = $(this).data('id');
